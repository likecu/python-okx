...
def create_1m_table_if_not_exists(table_name, db_config):
    """创建1分钟级别数据表（如果不存在）
    ...
    try:
        with conn.cursor() as cursor:
            ...
            """ % table_name
            cursor.execute(sql)
        print("1分钟级别表 %s 已准备就绪" % table_name)
        return True
    except Exception as e:
        print("创建1分钟级别表失败: %s" % e)
        return False


def save_1m_to_mysql(df, table_name, currency, db_config):
    """保存1分钟级别DataFrame到MySQL表
    ...
    try:
        ...
        conn.commit()
        print("成功写入 %d 条1分钟级别记录到MySQL表 %s (币种: %s)" % (len(df), table_name, currency))
        return True
    except Exception as e:
        print("写入MySQL失败: %s" % e)
        conn.rollback()
...
def load_1m_state(state_file_path):
    """加载保存的1分钟级别状态
...
def save_1m_state(state, state_file_path):
    """保存当前1分钟级别状态
...
def fetch_1m_data_with_retry(after_ts, inst_id, bar, limit, api_url, max_retries, retry_delay, random_delay):
    """带重试机制的1分钟级别API请求
...
def download_1m_single_currency(currency_config, db_config, api_config, table_name, save_path):
    """下载单个币种的1分钟级别历史数据
    ...
    try:
        ...
        bar = '1m'  # 使用1分钟级别（OKX API不支持15秒级别）
        time_range_days = currency_config.get('time_range_days', 30)  # 1分钟级别默认下载30天数据

        state_file_path = os.path.join(save_path, "download_1m_state_%s.json" % inst_id.replace('-', '_'))

        print("\n" + "=" * 60)
        print("开始下载 %s (%s) 1分钟级别历史数据" % (currency_config['name'], inst_id))
        print("注意：OKX API不支持15秒级别，1分钟是最接近的时间级别")
        print("=" * 60)

        os.makedirs(save_path, exist_ok=True)

        if not create_1m_table_if_not_exists(table_name, db_config):
            print("无法创建1分钟级别MySQL表，跳过 %s" % inst_id)
            return False

        state = load_1m_state(state_file_path)
        ...
        batch_data = []
        max_data_limit = 2000000  # 1分钟级别数据量较大，限制为200万条
        ...
        while True:
            try:
                ...
                response = fetch_1m_data_with_retry(
                ...
                print("已获取 %d 条1分钟级别记录 | 最早数据时间: %s | 总记录数: %d" % (len(page_data), oldest_time.strftime('%Y-%m-%d %H:%M:%S'), total_records))
                ...
                if total_records >= max_data_limit:
                    print("警告：已达到最大数据限制 (%d 条1分钟级别记录)" % max_data_limit)
                    break

                if len(batch_data) >= 500:  # 1分钟级别数据较密集，适中批处理大小
                    df = pd.DataFrame(
                        batch_data,
                    ...
                    if save_1m_to_mysql(df, table_name, inst_id, db_config):
                        print("成功保存 %d 条1分钟级别记录到MySQL" % len(batch_data))
                        ...
                        save_1m_state(state, state_file_path)

                        batch_data = []
                    else:
                        print("保存1分钟级别数据到MySQL失败，退出")
                        return False
                ...
                if batch_data:
                    ...
                    if save_1m_to_mysql(df, table_name, inst_id, db_config):
                        print("成功保存剩余 %d 条1分钟级别记录到MySQL" % len(batch_data))
                    else:
                        print("保存剩余1分钟级别数据到MySQL失败")
        ...
        if batch_data:
            ...
            if save_1m_to_mysql(df, table_name, inst_id, db_config):
                ...
                print("\n%s 1分钟级别数据已完全保存到MySQL表: %s" % (inst_id, table_name))
        ...
        print("下载 %s 1分钟级别数据时出错: %s" % (inst_id, str(e)))
        return False
    finally:
        if connection and connection.open:
            connection.close()
...
def main():
    ...
    try:
        ...
        for currency_config in enabled_pairs:
            if download_1m_single_currency(currency_config, db_config, api_config, table_name, save_path):
                success_count += 1
            else:
                fail_count += 1
