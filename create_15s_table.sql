-- 15秒级别行情数据表创建脚本
-- 用于存储OKX交易所15秒级别的多币种行情数据

USE okx_data;

-- 创建15秒级别历史数据表
CREATE TABLE IF NOT EXISTS sorted_history_15s (
  ...
  COMMENT='OKX交易所15秒级别多币种行情数据表';

-- 创建表完成提示
SELECT 'Table sorted_history_15s created successfully!' AS status;

-- 显示表结构验证
DESCRIBE sorted_history_15s;

-- 显示索引信息
SHOW INDEX FROM sorted_history_15s;