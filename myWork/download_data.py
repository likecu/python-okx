...
CONFIG = {
    ...
    "BAR": "15m",  # 时间粒度 (1s/1m/3m/5m/15m/30m/1H/2H/4H/6H/12H/1D等)
    "LIMIT": 100,  # 每页数据量 (最大100)
    "TIME_RANGE_DAYS": 730,  # 时间范围 (天数) - 2年的15min数据
    "MAX_DATA_LIMIT": 10000000,  # 最大数据量限制
    "SAVE_PATH": "./",  # 数据保存路径 (结尾带/)
    "TEMP_FILE": "temp_history_15m.csv",  # 临时数据文件名
    "FINAL_FILE": "sorted_history_15m.csv",  # 最终排序后文件名
    "STATE_FILE": "download_state_15m.json",  # 状态保存文件名
    "MAX_RETRIES": 5,  # API请求最大重试次数
    "RETRY_DELAY": 5,  # 重试前等待秒数 (基础值)
    "RANDOM_DELAY": 3,  # 随机延迟上限 (避免请求风暴)
    "MYSQL_TABLE": "sorted_history_15m"  # MySQL表名
}
